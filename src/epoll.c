...
{//
    struct epoll_event epv ={0,	{0}}; //event , epoll_data_t
    int	op;
    epv.data.ptr = ev;
...
{
    int	lfd=socket(AF_INET,SOCK_STREAM,0);//listenfd
    //  fcntl(lfd,F_SETFL,O_NONBLOCK);
    int flag = fcntl(lfd,F_GETFL,0);
    flag |= O_NONBLOCK;
    fcntl(lfd,F_SETFL,flag);

    //last is listen fd  ,  listenfd,  callback, args
    eventset(&g_events[MAX_EVENTS],lfd,acceptconn,&g_events[MAX_EVENTS]);

    eventadd(efd,EPOLLIN,&g_events[MAX_EVENTS]);//add to epoll
...
int main(int argc,char*argv[])
{
    ...
    if	(argc==2)
	port=atoi(argv[1]);
    g_efd=epoll_create(MAX_EVENTS+1); //create  epoll_fd
    if (g_efd<=0)
